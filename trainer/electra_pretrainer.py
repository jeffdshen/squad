"""Train and test methods for MLM-MLM pretraining.
    ...
    while epoch != args.num_epochs:
        ...
        with torch.enable_grad(), tqdm(total=len(train_loader.dataset)) as progress_bar:
            for x, y in train_loader:
                ...
                x, y = sample_mlm_pred(
                    model.module, x, y, scores, args.mlm_samples, args
                )
                ...
                progress_bar.set_postfix(epoch=epoch, NLL=loss_val, NLL_E=loss_val_e)
                ...
                if samples_till_eval <= 0:
                    ...
                    results, preds, preds_e = evaluate(model, dev_loader, device, args)
...
def sample_mlm_pred(model, x, y, scores, k, args):
    y = y.clone().detach()
    x = x.clone().detach()
    ...
    x = x.repeat(1, 1, k)
    x[mask.unsqueeze(-1)] = model.sample(
        scores[mask.unsqueeze(-1)], k, alpha=args.sample_temperature
    )
...
def evaluate(model, data_loader, device, args):
    ...
    nll_e_meter = stats.AverageMeter()
    acc_e_meter = stats.AverageMeter()
    ...
    with torch.no_grad():
        for x, y in data_loader:
            ...
            x, y = sample_mlm_pred(model.module, x, y, scores, 1, args)
            x = x.squeeze(0)
            _, loss_val, scores = forward(x, y, args, device, model)
            nll_e_meter.update(loss_val, batch_size)

            pred, ques, ans, acc = get_mlm_pred(model.module, x, y, scores, args)
            acc_e_meter.update(acc, batch_size)
            preds_e += zip(pred, ques, ans)

    model.train()

    results_list = [("NLL", nll_meter.avg), ("acc", acc_meter.avg)]
    results_list += [("NLL_E", nll_e_meter.avg), ("acc_e", acc_e_meter.avg)]
    results = OrderedDict(results_list)

    return results, preds, preds_e
...
def add_electra_mlm_args(parser):
    parser.add_argument(
